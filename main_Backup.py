...
bot = telebot.TeleBot('5627694754:AAEEYSQpUxkUz0m4_kDisV_k44AO_ZZ_s9w')
users_list = []
IsStart = False
...
def send_question(users_list, is_skip, send_name):
    ...
    if k == n:
        ...
        if is_skip == False:
            ...
            question_number = random.randint(0, len(QUESTIONS))
            return f'ещё один вопрос для {last_user_name}' + '\n' + str(question_number) + '. ' + QUESTIONS[question_number]
    else:
        return f'НЕ ТВОЯ ОЧЕРЕДЬ, {send_name}, ХВАТИТ ТЫКАТЬ'
